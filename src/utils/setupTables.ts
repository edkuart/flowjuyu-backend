...
const DEFAULT_COLLECTION_TEMPLATES = [
  {
    ...
    items_snapshot: [
      ...
      tplText({
        text: "Texturas nobles, siluetas limpias y una presencia pensada para vender valor percibido.",
        fontSize: 28,
        x: 94,
        y: 336,
        w: 420,
  ...
  },
  {
    ...
    items_snapshot: [
      ...
      tplText({
        text: "Lanzamiento premium con una pieza hero y narrativa minima.",
        fontSize: 28,
        x: 78,
        y: 294,
        w: 406,
  ...
  },
  {
    ...
    items_snapshot: [
      ...
      tplText({
        text: "Una portada pensada para escaparate digital, con mas aire y menos ruido visual.",
        fontSize: 32,
        x: 118,
        y: 336,
        w: 474,
  ...
  },
  {
    ...
    items_snapshot: [
      ...
      tplText({
        text: "Color, tension visual y una pieza central imposible de ignorar.",
        fontSize: 28,
        x: 84,
        y: 396,
        w: 446,
  ...
  },
  {
    ...
    items_snapshot: [
      ...
      tplText({
        text: "Hecho para lanzamientos rapidos, colecciones cortas y anuncios de alto impacto.",
        fontSize: 28,
        x: 78,
        y: 358,
        w: 404,
  ...
  },
  {
    ...
    items_snapshot: [
      ...
      tplText({
        text: "Origen, detalle y oficio convertidos en una coleccion de alto valor visual.",
        fontSize: 28,
        x: 82,
        y: 318,
        w: 464,
      ...
      tplText({
        text: "Hecho a mano · edicion limitada",
        fontSize: 32,
        x: 120,
        y: 646,
        w: 430,
      ...
      tplText({
        text: "Usa este bloque para explicar el origen de la coleccion, el material o la tecnica artesanal que la vuelve unica.",
        fontSize: 24,
        x: 122,
        y: 716,
        w: 560,
  ...
  },
  {
    ...
    items_snapshot: [
      ...
      tplText({
        text: "Pensada para telas, fibras y piezas que deben verse materiales, refinadas y profundamente humanas.",
        fontSize: 31,
        x: 100,
        y: 330,
        w: 486,
  ...
  },
  {
    ...
    items_snapshot: [
      ...
      tplText({
        text: "Formas suaves, composicion limpia y una presencia elegante para moda refinada.",
        fontSize: 27,
        x: 102,
        y: 316,
        w: 416,
      ...
      tplText({
        text: "Lineas puras · tonos suaves",
        fontSize: 22,
        x: 96,
        y: 1128,
        w: 240,
  ...
  },
  {
    ...
    items_snapshot: [
      ...
      tplText({
        text: "Una composicion limpia para lanzamientos serenos y producto protagonista.",
        fontSize: 26,
        x: 556,
        y: 348,
        w: 360,
  ...
  },
  {
    ...
    items_snapshot: [
      ...
      tplText({
        text: "Convierte una promo en una pieza de alto valor visual, sin perder sofisticacion.",
        fontSize: 28,
        x: 102,
        y: 334,
        w: 436,
  ...
  },
  {
    ...
    items_snapshot: [
      ...
      tplText({
        text: "Haz que la oferta se sienta exclusiva, no barata.",
        fontSize: 28,
        x: 98,
        y: 320,
        w: 360,
  ...
  },
  {
    ...
    items_snapshot: [
      ...
      tplText({
        text: "Tres piezas clave, una sola historia visual.",
        fontSize: 30,
        x: 112,
        y: 202,
        w: 390,
      ...
      tplText({
        text: "Ideal para portada de lineup o banner de coleccion.",
        fontSize: 27,
        x: 1188,
        y: 278,
        w: 258,
...
export async function setupCollectionTables(): Promise<void> {
  ...
  try {
    ...
    console.log(
      `  ✅ collection_templates seed (${seeded} inserted, ${DEFAULT_COLLECTION_TEMPLATES.length - seeded} already existed)`,
    );
  } catch (err: any) {
    console.error(
      "  ❌ collection_templates seed:",
      err?.original?.message ?? err?.message ?? err,
    );
  }
...
export async function setupConsentTables(): Promise<void> {
  ...
  try {
    policyColumns = await (queryInterface as any).describeTable(
      "policy_versions",
    );
  } catch {
    policyColumns = null;
  ...
  try {
    consentColumns = await (queryInterface as any).describeTable(
      "user_consents",
    );
  } catch {
    consentColumns = null;
...
export async function setupVideoStudioTables(): Promise<void> {
  console.log("🎬 setupVideoStudioTables: running…");
  ...
  await run(
    ...
    CREATE TABLE IF NOT EXISTS video_templates (
      ...
      name            TEXT NOT NULL,
      objective       TEXT NOT NULL CHECK (objective IN ('product','promo','live','collection')),
  ...
  await run(
    "video_projects index seller_id",
    `CREATE INDEX IF NOT EXISTS video_projects_seller_id_idx ON video_projects (seller_id, created_at DESC)`,
  );
  ...
  await run(
    ...
    CREATE TABLE IF NOT EXISTS video_assets (
      id           UUID PRIMARY KEY DEFAULT gen_random_uuid(),
      project_id   UUID NOT NULL REFERENCES video_projects(id) ON DELETE CASCADE ON UPDATE CASCADE,
      seller_id    INTEGER NOT NULL REFERENCES users(id) ON DELETE CASCADE ON UPDATE CASCADE,
      product_id   TEXT,
      asset_type   TEXT NOT NULL DEFAULT 'product_image' CHECK (asset_type IN ('product_image','custom_image','logo','text_overlay')),
  ...
  await run(
    "video_assets index project_id",
    `CREATE INDEX IF NOT EXISTS video_assets_project_id_idx ON video_assets (project_id, sort_order)`,
  );

  try {
    const { error } = await supabase.storage.createBucket("video-assets", {
      public: true,
    ...
    if (
      ...
      console.log("  ✅ video-assets bucket (ready)");
    } else {
      console.error("  ❌ video-assets bucket:", error.message);
    }
  } catch (err: any) {
    console.error("  ❌ video-assets bucket:", err?.message ?? err);
  }
  ...
  await run(
    ...
    CREATE TABLE IF NOT EXISTS video_generations (
      ...
      status                TEXT NOT NULL DEFAULT 'queued' CHECK (status IN ('queued','validating','generating','processing_output','completed','failed','cancelled','expired')),
      ...
      error_code            TEXT,
      error_message         TEXT,
      cost_estimated_cents  INTEGER DEFAULT 0,
  ...
  await run(
    "video_generations index project_id",
    `CREATE INDEX IF NOT EXISTS video_generations_project_id_idx ON video_generations (project_id, created_at DESC)`,
  );

  await run(
    "video_generations index seller_id",
    `CREATE INDEX IF NOT EXISTS video_generations_seller_id_idx ON video_generations (seller_id, created_at DESC)`,
  );

  // Seed default templates (idempotent via ON CONFLICT DO NOTHING)
  const defaultTemplates = [
    {
      ...
      duration_seconds: 10,
      prompt_template:
        "Muestra {product_name} con textura {texture}, detalle artesanal y caída de tela natural. Iluminación suave, fondo neutro, movimiento sutil.",
      style_config: { mood: "editorial", pacing: "slow", color_grade: "warm" },
    },
    {
      slug: "promo-whatsapp",
      name: "Promo WhatsApp",
      ...
      duration_seconds: 8,
      prompt_template:
        "{product_name} disponible ahora. Precio especial. Escríbenos por WhatsApp.",
      style_config: {
        mood: "energetic",
        pacing: "medium",
        color_grade: "vibrant",
    ...
    },
    {
      ...
      duration_seconds: 6,
      prompt_template:
        "Anuncio urgente: live hoy. {product_name} y más piezas exclusivas. No te lo pierdas.",
      style_config: { mood: "urgent", pacing: "fast", color_grade: "bold" },
    },
    {
      ...
      duration_seconds: 15,
      prompt_template:
        "Presentación editorial de nueva colección: {product_name}. Mood cultural, origen artesanal, secuencia de 3-6 piezas con transiciones suaves.",
      style_config: { mood: "editorial", pacing: "slow", color_grade: "muted" },
    },
    {
      slug: "producto-cuadrado",
      name: "Producto Cuadrado (Feed)",
      objective: "product",
      format: "1:1",
      duration_seconds: 10,
      prompt_template:
        "Vista de producto {product_name} para feed de Instagram. Detalle de textura y movimiento de tela.",
      style_config: { mood: "clean", pacing: "medium", color_grade: "neutral" },
    },
  ];

  for (const tpl of defaultTemplates) {
    await run(
      `video_template seed: ${tpl.slug}`,
      `
      INSERT INTO video_templates (slug, name, objective, format, duration_seconds, prompt_template, style_config, is_active, is_system)
      VALUES (
        '${tpl.slug}',
        '${tpl.name.replace(/'/g, "''")}',
        '${tpl.objective}',
        '${tpl.format}',
        ${tpl.duration_seconds},
        '${tpl.prompt_template.replace(/'/g, "''")}',
        '${JSON.stringify(tpl.style_config).replace(/'/g, "''")}'::jsonb,
        true,
        true
      )
  ...
  // Migrations: columnas agregadas después del diseño inicial
  await run(
    "video_generations add column model",
    `ALTER TABLE video_generations ADD COLUMN IF NOT EXISTS model TEXT NOT NULL DEFAULT 'default'`,
  );

  await run(
    "video_generations add column provider_model_combo_index",
    `CREATE INDEX IF NOT EXISTS video_generations_provider_model_idx ON video_generations (provider, model)`,
  );

  // Storage pipeline columns
  await run(
    "video_generations add column storage_path",
    `ALTER TABLE video_generations ADD COLUMN IF NOT EXISTS storage_path TEXT`,
  );

  await run(
    "video_generations add column file_size_bytes",
    `ALTER TABLE video_generations ADD COLUMN IF NOT EXISTS file_size_bytes INTEGER`,
  );

  // Note: the "videos" Supabase Storage bucket is created lazily on first upload
  // (inside uploadVideoFromUrl) — no blocking network call here at startup.

  // Credits wallet — stored in GTQ centavos (100 = Q1.00)
  await run(
    "vendedor_perfil col video_credits_gtq_cents",
    `ALTER TABLE vendedor_perfil ADD COLUMN IF NOT EXISTS video_credits_gtq_cents INTEGER NOT NULL DEFAULT 0`,
  );

  console.log("🎬 setupVideoStudioTables: done.");
}
...
export async function setupAiCreditsTables(): Promise<void> {
  console.log("🤖 setupAiCreditsTables: running…");

  // Balance on the seller profile (integer credits, not GTQ cents)
  await run(
    "vendedor_perfil col ai_credits_balance",
    `ALTER TABLE vendedor_perfil ADD COLUMN IF NOT EXISTS ai_credits_balance INTEGER NOT NULL DEFAULT 0`,
  );

  // Full ledger: every debit, credit, refund, and grant
  await run(
  ...
  await run(
    "ai_credit_transactions index ref",
    `CREATE INDEX IF NOT EXISTS ai_credit_tx_ref_idx ON ai_credit_transactions (ref_type, ref_id)`,
  );

  // Catalog of purchasable credit packages
  await run(
    "ai_credit_packages table",
    `
    CREATE TABLE IF NOT EXISTS ai_credit_packages (
      ...
      credits       INTEGER      NOT NULL,
      price_gtq     NUMERIC(8,2) NOT NULL,
      is_active     BOOLEAN      NOT NULL DEFAULT true,
      sort_order    INTEGER      NOT NULL DEFAULT 0,
      created_at    TIMESTAMPTZ  NOT NULL DEFAULT NOW()
    )
  ...
  for (const pkg of packages) {
    await run(
      `ai_credit_packages seed: ${pkg.slug}`,
      `
  ...
  await run(
    "ai_credit_purchase_requests table",
    `
    CREATE TABLE IF NOT EXISTS ai_credit_purchase_requests (
      id              UUID         PRIMARY KEY DEFAULT gen_random_uuid(),
      seller_id       INTEGER      NOT NULL REFERENCES users(id) ON DELETE CASCADE,
      package_id      INTEGER      NOT NULL REFERENCES ai_credit_packages(id),
      credits         INTEGER      NOT NULL,
      price_gtq       NUMERIC(8,2) NOT NULL,
      ...
      tx_id           UUID         REFERENCES ai_credit_transactions(id) ON DELETE SET NULL,
      created_at      TIMESTAMPTZ  NOT NULL DEFAULT NOW(),
      updated_at      TIMESTAMPTZ  NOT NULL DEFAULT NOW()
    )
    `,
  ...
  await run(
    "ai_credit_purchase_requests index seller",
    `CREATE INDEX IF NOT EXISTS ai_credit_req_seller_idx ON ai_credit_purchase_requests (seller_id, created_at DESC)`,
  );

  await run(
    "ai_credit_purchase_requests index status",
    `CREATE INDEX IF NOT EXISTS ai_credit_req_status_idx ON ai_credit_purchase_requests (status, created_at DESC)`,
  );

  await run(
    "ai_credit_purchase_requests add column provider",
    `ALTER TABLE ai_credit_purchase_requests ADD COLUMN IF NOT EXISTS provider VARCHAR(50)`,
  );

  await run(
    "ai_credit_purchase_requests add column provider_session_id",
    `ALTER TABLE ai_credit_purchase_requests ADD COLUMN IF NOT EXISTS provider_session_id VARCHAR(255)`,
  );

  await run(
    "ai_credit_purchase_requests add column provider_transaction_id",
    `ALTER TABLE ai_credit_purchase_requests ADD COLUMN IF NOT EXISTS provider_transaction_id VARCHAR(255)`,
  );

  await run(
    "ai_credit_purchase_requests add column payment_completed_at",
    `ALTER TABLE ai_credit_purchase_requests ADD COLUMN IF NOT EXISTS payment_completed_at TIMESTAMPTZ`,
  );

  await run(
    "ai_credit_purchase_requests unique provider_session",
    `CREATE UNIQUE INDEX IF NOT EXISTS ai_credit_req_provider_session_uidx ON ai_credit_purchase_requests (provider_session_id) WHERE provider_session_id IS NOT NULL`,
  );

  await run(
    "ai_credit_purchase_requests unique provider_transaction",
    `CREATE UNIQUE INDEX IF NOT EXISTS ai_credit_req_provider_transaction_uidx ON ai_credit_purchase_requests (provider_transaction_id) WHERE provider_transaction_id IS NOT NULL`,
  );

  console.log("🤖 setupAiCreditsTables: done.");
}
